...
export default class CoreLoginReconnectPage implements OnInit, OnDestroy {
    ...
    /**
     * Try biometric authentication if enabled for this site.
     */
    protected async tryBiometricAuth(): Promise<void> {
        try {
            // Check if biometric is available and enabled for this site
            const isEnabled = await this.biometricAuth.isAvailableAndEnabled(this.siteId);

            if (!isEnabled) {
                // Biometric not enabled, show normal login form
                return;
            }

            // Authenticate with biometric
            await this.biometricAuth.authenticate(this.siteId);

            // Biometric authentication successful - reconnect with existing token
            const url = this.site.getURL();
            ...
            // Set site as logged in
            await CoreSites.login(this.siteId);
            ...
            if (this.biometricAuth.isUserCancelled(error)) {
                // User cancelled - silently show password form (already visible)
                return;
            }

            // Get error message
            const errorMessage = this.biometricAuth.getErrorMessage(error);

            if (errorMessage) {
                // Show error and fallback to password
                CoreAlerts.showError(errorMessage);
            }

            // If biometric fails, user can use password form
        }
    }
